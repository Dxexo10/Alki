/**
 * Alki — Shared Mobile App Interactions & Icons (Holst Design System)
 * Handles:
 *  - View Transitions API navigation (forward/back slide simulation)
 *  - Custom SVG Icon registry (Phosphor/Lucide stroke style, 1.75px, currentColor)
 *  - Interactive photo carousels (scroll-snap + reactive dots)
 *  - Scroll reveal (Intersection Observer)
 *  - Instant active feedback on buttons
 */

(function () {
  'use strict';

  /* ── 1. SVG ICONS REPOSITORY (Stroke-based, currentColor, 1.75px stroke-width) ── */
  const ICONS = {
    // Navigation
    home: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9.5L12 3l9 6.5V20a1.5 1.5 0 0 1-1.5 1.5H4.5A1.5 1.5 0 0 1 3 20V9.5z"/><path d="M9 21V12h6v9"/></svg>`,
    ...
    chat: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.5 8.5 0 0 1 8 8v.5z"/></svg>`,
    clock: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><polyline points="12 6 12 12 16 14"/></svg>`,
    ...
    qr: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>`,
    ...
    brush: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M9.06 11.9l8.07-8.06a2.85 2.85 0 1 1 4.03 4.03l-8.06 8.08"/><path d="M7.07 14.94l-3.53 3.54a2 2 0 0 0 0 2.83l.71.71a2 2 0 0 0 2.83 0l3.53-3.54"/><line x1="16" y1="5" x2="19" y2="8"/></svg>`,
    sparkles: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l1.91 5.89L20 10.8l-4.55 3.96L16.82 21 12 17.27 7.18 21l1.37-6.24L4 10.8l6.09-1.91L12 3z"/></svg>`,
    hammer: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M15 12l-8.5 8.5a2.12 2.12 0 0 1-3-3L12 9"/><path d="M17.64 4.36a3 3 0 0 0-4.24 0L12 5.76 18.24 12l1.4-1.4a3 3 0 0 0 0-4.24z"/></svg>`,
    key: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 2l-2 2m-1.5 1.5L14 9l-3-3-6 6a5.5 5.5 0 1 0 7.78 7.78L21 10.5V7l-2-2z"/><circle cx="7.5" cy="16.5" r="1.5"/></svg>`,
    ...
    creditCard: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>`,
  ...
  window.rsIcon = function (name, className = '') {
  ...
  /* ── 2. TOAST / SNACKBAR ──
     Brief success confirmation that auto-dismisses. Positioned above the
     fixed bottom bar of the current screen (measured at call time). */
  window.rsShowToast = function (message, ms) {
    const host = document.querySelector('.phone') || document.body;
    const bar = document.querySelector('.bottom-action-bar, .tab-bar, .bottom-bar, .chat-input-bar');
    const bottom = (bar ? Math.ceil(bar.getBoundingClientRect().height) : 0) + 20;

    const t = document.createElement('div');
    t.className = 'rs-toast';
    t.setAttribute('role', 'status');
    t.style.bottom = bottom + 'px';
    t.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg><span></span>';
  ...
  /* ── 2. VIEW TRANSITIONS API NAVIGATION ── */
  ...
  function setupViewTransitions() {
    document.addEventListener('click', (e) => {
      ...
      // Only intercept local html links, not hashes or externals
      if (!href || href.startsWith('#') || href.startsWith('http') || href.startsWith('mailto')) {
        return;
      }

      e.preventDefault();
      const isBack = link.hasAttribute('data-nav-back') ||
                     link.classList.contains('back') ||
                     link.classList.contains('nav-back');
      navigateWithTransition(href, isBack);
    });
  }

  /* ── 4. CAROUSEL INTERACTIVITY (Scroll-snap + reactive dots) ── */
  ...
  /* ── 6. DYNAMIC BOTTOM SPACE (measure the fixed bottom bar) ──
     Measures the real height of the fixed bottom bar present on the page
     (.bottom-action-bar / .tab-bar / .bottom-bar) and exposes it as
     --dynamic-bottom-space on the scroll container, so .screen-content's
     padding-bottom always clears the bar regardless of its content/height. */
  function setupDynamicBottomSpace() {
    const scroller = document.querySelector('.screen-content, .content-area');
    if (!scroller) return;

    const bar = document.querySelector('.bottom-action-bar, .tab-bar, .bottom-bar');
    const SAFETY = 24;

    function measure() {
      const h = bar ? Math.ceil(bar.getBoundingClientRect().height) : 0;
      scroller.style.setProperty('--dynamic-bottom-space', (h > 0 ? h + SAFETY : 0) + 'px');
    }
    ...
    // Re-measure shortly after load in case fonts/late layout change the bar height.
